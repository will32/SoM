    ...
    with torch.autocast(device_type='cuda', dtype=torch.float16):
        ...
        for entry in image:
            ...
            if model_name == 'semantic-sam':
                model = model_semsam
                output, mask = inference_semsam_m2m_auto(model, imageFile, level, text, text_part, text_thresh, text_size, hole_scale, island_scale, semantic, label_mode=label_mode, alpha=alpha, anno_mode=anno_mode, *args, **kwargs)

            elif model_name == 'sam':
                model = model_sam
                if mode == "Automatic":
                    output, mask = inference_sam_m2m_auto(model, imageFile, text_size, label_mode, alpha, anno_mode)
                ...
                model = model_seem
                if mode == "Automatic":
                    output, mask = inference_seem_pano(model, imageFile, text_size, label_mode, alpha, anno_mode)
                # elif mode == "Interactive":
            ...
            res = request_gpt4v("Describe the image", Image.fromarray(output))
            history_texts.append(res)

        return [(img, history_texts[i]) for i, img in enumerate(history_images)]
...
class ImageMask(gr.components.File):
    """
...
demo = gr.Blocks()
image = ImageMask(label="Input", type="file", brush_radius=20.0, brush_color="#FFFFFF", height=512)
...
image_out = gr.Gallery(value=history_images, show_label=True, preview=True )
